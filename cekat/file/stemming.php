...
function cekKamus($kata){ 
	include "../koneksi.php";
	$sql = mysqli_query($conn, "SELECT * from kata_dasar where kata_dasar ='$kata' LIMIT 1");
...
function cekKamusdi($kata){
	include "../koneksi.php";
	$sql = mysqli_query($conn, "SELECT * from ket_tempat where ket_tempat ='$kata' LIMIT 1");
	$result = mysqli_num_rows($sql);
...
function stemdi($kata){
	$kataAsal=$kata;
	$cekKata=cekKamusdi($kata);
	if($cekKata == true){ // Cek Kamus
	return $kata;
}
}
